...
######Формат файла "check.txt"
Первые несколько строк указываюстся товары (до строки "***params***"). Формат строк *Имя товара*;*Цена*;*Кол-во*.
Затем обязательно передать строку "***params***".
*Далее:*
  1. Первая строка: "PaymentType; nalichnimi / credit" - этот параметр выставляет тип оплаты:  Наличными / Картой.
  2. Вторая строка: "Summ;*Сумма наличных полученых от клиента*. - Можно не задавать если в первой строке указан тип оплаты картой.
